...
**Source baseline:** Phase 2B authentication and session-ownership implementation
**Reconciliation scope:** RFC-0005 and GitHub issue
...
|---|---|---|---|---|---|:---:|:---:|:---:|:---:|
| AG-001 | Resolved | RFC-0005 is accepted. `src/security.py` validates OIDC principals; `src/main.py` applies identity to API routes; `src/memory.py` binds sessions atomically to tenant and subject; filtered list and cross-owner tests pass. | No remaining liability under this entry for the supported single-instance boundary. Shared-state limits remain AG-008 and AG-009. | None | Isolation, Control | Yes | No | No | No |
...
| AG-006 | Partial | `src/config.py` now owns logical Model 1/Model 2 keys, exact aliases, physical NVIDIA model IDs, generation parameters, and token ceilings. `src/sc_evm.py` uses Model 1 for reformulation; `src/agent.py` uses both roles for candidates and Model 2 for synthesis. | RFC-0004 must still define capability discovery, adaptive role selection, escalation, and provider-neutral routing. Configurable aliases are deterministic routing, not task-aware orchestration. | Medium | Control | Yes | No | No | No |
| AG-007 | Partial | Production settings reject wildcard or non-HTTPS origins, OIDC mode disables credentialed-cookie CORS, and development retains local origins. | Narrow production methods/headers and add deployed negative CORS evidence before public pilot. | Medium | Control, Isolation | Yes | No | Yes | Yes |
...
| AG-015 | Resolved | RFC-0005 requires privileged diagnostics. `src/main.py` enforces `scevm:diagnostic` before session or provider work, and negative scope tests return `403`. | No remaining liability under this entry. Production global diagnostic mode is rejected by configuration validation. | None | Isolation | Yes | No | No | No |
...
- **Resolved:** AG-001, AG-015, and AG-016.
- **Partial with explicit remaining work:** AG-004, AG-005, AG-006, AG-007, AG-008,
  AG-010, AG-013, AG-014, and AG-017.
- **Public-pilot blockers:** AG-007, AG-009, and AG-014.
