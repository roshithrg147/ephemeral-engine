...
async def main():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            resp_init_a = await client.post(f"{BASE_URL}/api/session/initialize", json={"session_id": session_a})
            assert resp_init_a.json()["status"] == "success"

            log_event("InitSession", f"Initializing: {session_b}")
            resp_init_b = await client.post(f"{BASE_URL}/api/session/initialize", json={"session_id": session_b})
            assert resp_init_b.json()["status"] == "success"

            # 3. Simulate concurrent speed-typing operations injecting different context
            log_event("ConcurrentInjections", "Firing concurrent context prompts to Session A and Session B")
            ...
            log_event("InjectionsCompleted", {
                "session_a_response": response_a,
                "session_b_response": response_b
            })
            ...
            log_event("IsolationCheckComplete", {
            ...
            assertions_log.append({"assertion": "SessionA Knows Its Secret", "passed": bool(assert_1)})
            assertions_log.append({"assertion": "SessionA Wont Leak SessionB Secret", "passed": bool(assert_2)})
            assertions_log.append({"assertion": "SessionB Knows Its Secret", "passed": bool(assert_3)})
            assertions_log.append({"assertion": "SessionB Wont Leak SessionA Secret", "passed": bool(assert_4)})
            ...
            log_event("IsolationVerificationPassed", "Strict multi-tenant isolation validated. Zero leakages.")
            ...
            wipe_check_a = await consume_sse_tokens(client, session_a, "What was the primary capital city for the region of Alpha?")
            still_active_b = await consume_sse_tokens(client, session_b, "What was the primary capital city for the region of Beta?")

            log_event("ResourceCleanlinessComplete", {
                "session_a_post_burn": wipe_check_a,
                "session_b_post_burn": still_active_b
            })

            assert_5 = "GargantuanApple77" not in wipe_check_a
            assert_6 = "MicroscopicBanana88" in still_active_b

            assertions_log.append({"assertion": "SessionA Forgot Secret Post-Burn", "passed": bool(assert_5)})
            assertions_log.append({"assertion": "SessionB Remains Intact Post-Burn", "passed": bool(assert_6)})
            ...
            log_event("WipeVerificationPassed", "Burn verification successful. RAM footprint and Chroma vector space cleared.")
