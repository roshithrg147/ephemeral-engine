...
from src.session_rehydration_hook import load_history, rehydrate_session, wait_for_backend

PORT = 8091
API_URL = f"http://127.0.0.1:{PORT}"
logger = logging.getLogger("TestSessionRehydration")
...
async def run_server(server: BackgroundUvicornServer):
    try:
        ...
        logger.exception("Background uvicorn server failed", extra={"error": str(exc)})
        raise


@unittest.skipUnless(
    os.getenv("SC_EVM_RUN_NETWORK_TESTS") == "1",
    "set SC_EVM_RUN_NETWORK_TESTS=1 to run localhost rehydration tests",
)
