...
async def consume_sse_stream(client: httpx.AsyncClient, session_id: str, prompt: str) -> str:
    ...
    async with client.stream(
        "POST", f"{BASE_URL}/api/agent/query", json=payload, timeout=400.0
    ) as response:
...
async def run_torture_test():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            log_event("InjectState", "Seeding Tenant A context tracking anchors.")
            await client.post(
            ...
            log_event("InjectState", "Seeding Tenant B context tracking anchors.")
            await client.post(
                f"{BASE_URL}/api/session/message",
                json={
            ...
            assertions.append(
                {"assertion": "SessionA Wont Leak SessionB Secret", "passed": assert_a2}
            )
            ...
            assertions.append(
                {"assertion": "SessionB Wont Leak SessionA Secret", "passed": assert_b2}
            )
            ...
            res_a_post_burn = await consume_sse_stream(
                client, session_a, "What is my validation ID?"
            )
            res_b_post_burn = await consume_sse_stream(
                client, session_b, "What is my validation ID?"
            )

            assert_burn_a = "ALPHA-PLATINUM-9982" not in res_a_post_burn
            assert_burn_b = "BETA-TITANIUM-1104" in res_b_post_burn
            assertions.append(
                {"assertion": "SessionA Forgot Secret Post-Burn", "passed": assert_burn_a}
            )
            assertions.append(
                {"assertion": "SessionB Remains Intact Post-Burn", "passed": assert_burn_b}
            )
            ...
            log_event(
                ...
                {"session_a_post_burn": res_a_post_burn, "session_b_post_burn": res_b_post_burn},
            )
    finally:
