...
def safe_copy(text):
    ...
    try:
        pyperclip.copy(text)
    except (PyperclipException, Exception) as e:
        log_error("clipboard_service.safe_copy", str(e))
...
def _run_window_command(command):
    ...
    return output.decode("utf-8").strip().lower()


class AIService:
    def __init__(self, agent_orchestrator, memory_manager=None):
    ...
    def transform_code(self, text: str, transform_type: str) -> str:
        ...
        try:
            response = self._generate_response(f"{prompt_prefix}{text}")
            return response.text.strip()
        except Exception as e:
            return f"AI Transformation Error: {str(e)}"


def _decrypt_history_item(cipher, encrypted_item):
    return cipher.decrypt(encrypted_item).decode("utf-8", errors="ignore")


class ClipboardService:
    _template_file = os.path.expanduser("~/.config/anthropic-agent/templates.json")
    ...
    def __init__(
        ...
        self.ai_service = AIService(self.agent_orchestrator, memory_manager=memory_manager)
    ...
    def _default_templates(self):
        return [
            ...
            {
                "title": "Reject Email (Polite)",
                "content": "Thank you for reaching out. Unfortunately, we are not able to proceed at this time.",
            },
        ]

    def _ensure_template_store(self):
        template_dir = os.path.dirname(self._template_file)
        if not os.path.exists(template_dir):
            os.makedirs(template_dir, exist_ok=True)
        if not os.path.exists(self._template_file):
            with open(self._template_file, "w", encoding="utf-8") as f:
                json.dump(self._default_templates(), f, indent=4)

    def _load_templates(self):
        self._ensure_template_store()
        try:
            with open(self._template_file, encoding="utf-8") as f:
                self.templates = json.load(f)
        except Exception as e:
            logger.error(f"Failed to load templates: {e}")

    def _store_clip_record(
        self,
        stored_text: str,
        *,
        ai_payload: str | None = None,
        sync_payload: str | None = None,
    ) -> bool:
        current_hash = self._get_hash(stored_text)

        with self.lock:
            if self._last_clip_hash == current_hash:
                return False
            ...
            if self.ai_enabled and ai_payload is not None:
                self.ai_queue.put((current_hash, ai_payload))

        if sync_payload is not None and self.sync_service.enabled:
            self.sync_service.push(sync_payload)

        self._push_update_to_ui()
        return True

    def _resolve_active_window_name(self):
        import platform

        sys_name = platform.system()
        if sys_name == "Darwin":
            return _run_window_command(
                [
                    "osascript",
                    "-e",
        ...
        if sys_name == "Linux":
            commands = [
                [
                    ...
                    "global.get_window_actors().map(a => a.get_meta_window()).find(w => w.has_focus()).get_title()",
                ],
            ...
            for command in commands:
                try:
                    output = _run_window_command(command)
                    if command[0] == "gdbus":
                        return output.split("'")[1]
                    return output
                except Exception as e:
                    log_error(f"clipboard_service.{command[0]}", str(e))
            return ""
        ...
        if sys_name == "Windows":
            try:
                ...
                log_error("clipboard_service.pygetwindow", str(e))
    ...
    def stop(self):
        ...
        if self._ai_thread and self._ai_thread.is_alive():
            self._ai_thread.join(timeout=0.1)
    ...
    def ingest_remote_clip(self, text: str, _timestamp: float):
        if (
            not text
            or len(text.encode("utf-8", errors="ignore")) > self.max_clip_size
            or self._is_sensitive_or_invalid(text)
        ):
            return
        self._store_clip_record(text, ai_payload=text, sync_payload=text)

    def add_external_clip(self, text: str):
        if not text or len(text.encode("utf-8", errors="ignore")) > self.max_clip_size:
            return
        ...
        if not self._is_sensitive_or_invalid(text):
            ...
            if not self._store_clip_record(
                stored_text,
                ai_payload=None if is_agent else stored_text,
                sync_payload=None if is_agent else text,
            ):
                return
    ...
    def _is_terminal_or_vault_active(self):
        try:
            window_name = self._resolve_active_window_name()
            if any(term in window_name for term in self.sensitive_apps):
                return True
        except Exception as e:
            log_error("clipboard_service._is_terminal_or_vault_active", str(e))
        return False
        ...
        while self._running:
            try:
                ...
                if len(current_clip.encode("utf-8", errors="ignore")) > self.max_clip_size:
                    self._last_clip_hash = self._get_hash(current_clip)
                    continue
                if not self._is_sensitive_or_invalid(current_clip):
                    self._store_clip_record(
                        current_clip,
                        ai_payload=current_clip,
                        sync_payload=current_clip,
                    )
            except Exception as e:
        ...
        with self.lock:
            if 0 <= index < len(self.history):
                try:
                    ...
                    plaintext = _decrypt_history_item(self._cipher, encrypted_item)
    ...
    def transform_item(self, index: int, transform_type: str):
        ...
        with self.lock:
            if 0 <= index < len(self.history):
                try:
                    encrypted_item = self.history[index]
                    plaintext = self._cipher.decrypt(encrypted_item).decode(
                        "utf-8", errors="ignore"
                    )
                except Exception as e:
        ...
        try:
            if transform_type == "json":
                ...
                if parts:
                    transformed_text = parts[0].lower() + "".join(
                ...
                try:
                    transformed_text = base64.b64decode(plaintext.strip()).decode("utf-8")
                except Exception:
                    return
    ...
    def copy_to_clipboard(self, text: str):
        try:
            ...
            log_error("clipboard_service.copy_to_clipboard", str(e))
