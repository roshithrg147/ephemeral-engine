...
def verify_environment():
    ...
    if system.startswith("linux"):
        ...
        if not (which("xclip") or which("xsel") or which("wl-clipboard")):
            logger.warning("System dependency missing: xclip, xsel, or wl-clipboard is required on Linux. Pyperclip might fail.")
    elif system == "darwin":
        try:
            import tkinter
        except ImportError:
...
class ClipboardConsumerApp:
    def __init__(self, root: tk.Tk, agent_orchestrator):
        self.root = root
        self.root.configure(bg=THEME["bg"])
        self.update_queue = queue.Queue()
        self.service = ClipboardService(agent_orchestrator, update_queue=self.update_queue)
    ...
    def _setup_ui(self):
        ...
        tk.Label(self.settings_frame, text="GLOBAL HOTKEYS:", font=THEME["font_bold"], bg=THEME["bg"], fg=THEME["fg"]).pack(anchor=tk.W, padx=10, pady=(15, 0))
        ...
        self.sync_toggle = ttk.Checkbutton(self.sync_frame, text="ENABLE E2EE SYNC (BYOB)", variable=self.sync_enabled_var, command=self._on_sync_config_change, style="TCheckbutton")
    ...
    def _on_sync_config_change(self):
        ...
        self.sync_status_label.config(text=f"STATUS: {status}", fg=THEME["status_ok"] if "Connected" in status else THEME["status_err"])
