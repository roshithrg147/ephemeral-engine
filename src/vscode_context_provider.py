...
EXCLUDED_DIRS = {'.git', 'node_modules', '.venv', '__pycache__', '.pytest_cache', '.mypy_cache', 'dist', 'build', '.idea', '.vscode'}
INCLUDED_EXTENSIONS = {
    '.py', '.js', '.ts', '.tsx', '.jsx', '.html', '.css', '.json', '.md', 
    '.rs', '.go', '.c', '.cpp', '.h', '.hpp', '.java', '.sh', '.yaml', '.yml',
    '.toml', '.sql', '.ini', '.cfg'
}
...
class WorkspaceScanner:
    ...
    def __init__(self, db_path: str, collection_name: str = "workspace_context"):
        ...
        logger.info(f"Collection '{collection_name}' initialized. Current count: {self.collection.count()}")
    ...
    @staticmethod
    def chunk_file_content(content: str, max_chunk_size: int = 1000, overlap: int = 100) -> List[str]:
        """
    ...
    def update_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            metadatas = [{"file_path": abs_path, "chunk_index": i, "total_chunks": len(chunks)} for i in range(len(chunks))]
    ...
    def scan_local_workspace(self, root_path: str) -> int:
        ...
        logger.info(f"Found {len(file_list)} potential source files to index.")
        return self.index_files(file_list)
...
def main():
    ...
    parser.add_argument("--db-path", type=str, default=os.path.expanduser("~/.config/anthropic-agent/workspace_db"), help="ChromaDB persistent path")
    parser.add_argument("--collection-name", type=str, default="workspace_context", help="ChromaDB collection name")
    parser.add_argument("--index-files", type=str, help="Path to a JSON file containing a list of files to index, or '-' to read from stdin")
    parser.add_argument("--update", type=str, help="Absolute path of a file to update in the index (e.g. on file save)")
    parser.add_argument("--query", type=str, help="Query the workspace index for relevant context")
    parser.add_argument("--scan-root", type=str, help="Absolute path of the workspace root to scan and index directly (fallback/standalone mode)")
    parser.add_argument("--n-results", type=int, default=5, help="Number of results to return for query")
    ...
    if args.index_files:
        if args.index_files == '-':
            ...
            if not os.path.exists(args.index_files):
                logger.error(f"Index file list not found: {args.index_files}")
                sys.exit(1)
            with open(args.index_files, 'r', encoding='utf-8') as f:
                data = f.read().strip()
                
        try:
            file_paths = json.loads(data)
        except json.JSONDecodeError:
            file_paths = [line.strip() for line in data.splitlines() if line.strip()]
