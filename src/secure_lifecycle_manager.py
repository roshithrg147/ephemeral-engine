    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") == "success":
                ...
                return True
        logger.error(f"FastAPI session burn failed. Status code: {response.status_code}, Response: {response.text}")
        return False
...
def purge_local_temp_previews() -> int:
    ...
    try:
        ...
        # Try to truncate/write empty list as fallback
        try:
...
def main():
    ...
    parser.add_argument("--api-url", type=str, default="http://127.0.0.1:8000", help="FastAPI microservice base URL")
